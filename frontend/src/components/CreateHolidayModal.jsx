...
export default function CreateHolidayModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiFetch('/holiday', {
        method: 'POST',
        headers: {
      ...
      toast.error(error.message || 'An error occurred while creating the holiday');
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform transition-all scale-100 duration-300">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <h2 className="text-xl font-bold text-gray-800">New Holiday</h2>
          <button 
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="pt-2 flex gap-3">
            ...
            <button
              ...
              className="flex-1 py-3 px-4 bg-[#5069E5] text-white rounded-xl font-semibold hover:bg-[#3d52c7] transition-all shadow-lg shadow-blue-200 disabled:opacity-70 disabled:cursor-not-allowed"
            >

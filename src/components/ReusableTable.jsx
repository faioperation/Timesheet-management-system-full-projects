 ...
 * @param {Function} onRowClick - Callback when a row is clicked (optional)
 */
const ReusableTable = ({
  columns = [],
  data = [],
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentItems.map((row, rowIndex) => (
                <tr
                  key={row.id || rowIndex}
                  className={`${stripedRows && rowIndex % 2 === 1 ? 'bg-gray-50' : 'bg-white'
                ...
                >
                  {columns.map((column, colIndex) => (
                    <td
                      ...
                      className={`px-6 py-4 whitespace-nowrap text-sm ${column.cellClassName || 'text-gray-700'
                        } ${column.className || ''}`}
                    >
                      {column.render
              ...
              {getPageNumbers().map((page, index) => {
                ...
                return (
                  <button
                    ...
                    className={`px-4 py-2 border border-gray-300 text-sm font-medium transition-colors ${currentPage === page

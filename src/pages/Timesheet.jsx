...
export default function Timesheet() {
  ...
  const handleDownloadFile = async (attachmentId, originalFilename) => {
    try {
      console.log('Downloading attachment ID:', attachmentId);
      
      // Use API endpoint to download file
      const response = await apiFetch(`/attachment/${attachmentId}/download`, {
        method: 'GET',
      });
          ...
          <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
            ...
            {isDetailsLoading ? (
              ...
              <div className="space-y-4 text-sm">
                ...
                {/* Attachments Section */}
                {selectedTimesheetDetails?.attachments?.length > 0 && (
                  <div>
                    <p className="text-gray-500 mb-2">Attachments ({selectedTimesheetDetails.attachments.length})</p>
                    <div className="space-y-2 max-h-56 overflow-auto">
                      {selectedTimesheetDetails.attachments.map((attachment, idx) => (
                        <div key={idx} className="flex items-center justify-between px-3 py-2 bg-gray-50 rounded border border-gray-200">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-gray-700 truncate" title={attachment.original_filename}>
                          ...
                          <div className="flex items-center gap-2 ml-2">
                            ...
                            {(attachment.file_type?.startsWith('image/') || attachment.file_type === 'application/pdf') && (
                              <button
                                onClick={() => window.open(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/storage/${attachment.file_path}`, '_blank')}
